...
import { v4 } from 'uuid';
import { isUndefined as isUndefined$1, isString, find, isNil, flatten, values, isObject, filter, isArray, isFunction, without, forEach, groupBy, sortBy, keys, assign, bind, set, pick, findIndex, has } from 'min-dash';
...
import { useState, useEffect, useMemo } from '@bpmn-io/properties-panel/preact/hooks';
  ...
  set(templates) {
    ...
    templates.forEach(template => {
      const id = template.id,
        version = isUndefined$1(template.version) ? '_' : template.version;
      ...
      if (isUndefined$1(latestVerions) || template.version > latestVerions) {
...
/**
 * Copied and adapted from existing form-js#Sanitizer
...
function isValidAttribute(lcTag, lcName, value) {
  const FORM_ELEMENT = getFormElement();
...
function getFormElement() {
  return getFormElement.formElement || (getFormElement.formElement = document.createElement('form'));
}
