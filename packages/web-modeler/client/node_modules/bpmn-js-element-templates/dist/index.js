...
function isFeel$1(value) {
  return minDash.isString(value) && value.trim().startsWith('=');
}
...
function isPropertyAllowed(element, property) {
  ...
  if (type === 'bpmn:Message#zeebe:subscription#property' && binding.name === 'correlationKey' && ModelUtil.is(element, 'bpmn:StartEvent') && !DiUtil.isEventSubProcess(element.parent)) {
    return false;
  }
  return true;
}
...
function canHaveReferencedElement(element) {
  ...
  return ModelUtil.isAny(element, ['bpmn:ReceiveTask', 'bpmn:SendTask']);
}
...
function TemplateGroupButtons({
  ...
  const elementTemplates = bpmnJsPropertiesPanel.useService('elementTemplates');
  ...
  if (templateState.type === 'NO_TEMPLATE') {
    ...
    return jsxRuntime.jsx(DeprecatedTemplate, {
      element: element,
      templateState: templateState
    });
  } else if (templateState.type === 'INCOMPATIBLE_TEMPLATE') {
    return jsxRuntime.jsx(IncompatibleTemplate, {
      element: element
    });
...
function AppliedTemplate({
  ...
  const translate = bpmnJsPropertiesPanel.useService('translate'),
    elementTemplates = bpmnJsPropertiesPanel.useService('elementTemplates');
  const menuItems = [{
...
function DeprecatedTemplate({
  ...
  const translate = bpmnJsPropertiesPanel.useService('translate'),
    elementTemplates = bpmnJsPropertiesPanel.useService('elementTemplates');
  const menuItems = [{
    entry: jsxRuntime.jsx(DeprecationWarning, {
      template: template
    })
  }, {
    separator: true
...
function DeprecationWarning({
  ...
  return jsxRuntime.jsxs("div", {
    ...
    children: [message, documentationRef && jsxRuntime.jsxs(jsxRuntime.Fragment, {
      children: ["\xA0", jsxRuntime.jsx("a", {
        href: documentationRef,
        children: jsxRuntime.jsx(DocumentationIcon, {})
      })]
    })]
  });
}
...
function IncompatibleTemplate({
  ...
  const translate = bpmnJsPropertiesPanel.useService('translate'),
    elementTemplates = bpmnJsPropertiesPanel.useService('elementTemplates');
  const menuItems = [{
    entry: jsxRuntime.jsx(IncompatibleText, {})
  }, {
    separator: true
  }, {
    entry: translate('Unlink'),
...
function TemplateName({
  ...
  return name ? jsxRuntime.jsx(TextEntry, {
    id: id,
...
function PropertyTooltip(props) {
  ...
  return tooltip && jsxRuntime.jsx(Markup, {
...
function usePropertyAccessors(bpmnFactory, commandStack, element, property) {
  const directSet = hooks.useCallback(propertySetter$1(bpmnFactory, commandStack, element, property), [bpmnFactory, commandStack, element, property]);
  const directGet = hooks.useCallback(propertyGetter$1(element, property), [element, property]);
  const [isFeelEnabled, setIsFeelEnabled] = hooks.useState(feelEnabled(property, directGet()));
  const handleFeelToggle = hooks.useCallback(value => {
...
function TextAreaProperty$1(props) {
  ...
  const bpmnFactory = bpmnJsPropertiesPanel.useService('bpmnFactory'),
    commandStack = bpmnJsPropertiesPanel.useService('commandStack'),
    debounce = bpmnJsPropertiesPanel.useService('debounceInput'),
    translate = bpmnJsPropertiesPanel.useService('translate');
...
function useServiceIfAvailable(service, fallback) {
  const resolved = bpmnJsPropertiesPanel.useService(service, false);
  if (!resolved) {
    return fallback;
  }
  return resolved;
...
function withTooltipContainer(Component) {
  return props => {
    const tooltipContainer = hooks.useMemo(() => {
      const config = bpmnJsPropertiesPanel.useService('config');
...
function BooleanProperty$1(props) {
  ...
  const bpmnFactory = bpmnJsPropertiesPanel.useService('bpmnFactory'),
    ...
    debounce = bpmnJsPropertiesPanel.useService('debounceInput'),
    translate = bpmnJsPropertiesPanel.useService('translate');
  const Component = feel === 'optional' ? propertiesPanel.FeelCheckboxEntry : propertiesPanel.CheckboxEntry;
...
function NumberProperty(props) {
  ...
  const Component = feel === 'optional' ? propertiesPanel.FeelNumberEntry : propertiesPanel.NumberFieldEntry;
  const bpmnFactory = bpmnJsPropertiesPanel.useService('bpmnFactory'),
    commandStack = bpmnJsPropertiesPanel.useService('commandStack'),
    debounce = bpmnJsPropertiesPanel.useService('debounceInput'),
    translate = bpmnJsPropertiesPanel.useService('translate');
  ...
  const validate = hooks.useCallback(value => {
    if (shouldCastToFeel(property) && minDash.isNumber(value) && value.toString().includes('e')) {
...
function createCustomEntry$1(id, element, property) {
  ...
  if (feel === 'required') {
    return {
      ...
      isEdited: createIsEdited(propertiesPanel.isFeelEntryEdited, element, property),
      property
    };
  }
  if (type === 'Number') {
    return {
      ...
      isEdited: createIsEdited(propertiesPanel.isNumberFieldEntryEdited, element, property),
      property
    };
  }
  if (type === 'Boolean') {
    return {
      ...
      isEdited: createIsEdited(propertiesPanel.isCheckboxEntryEdited, element, property),
      property
    };
  }
  if (type === 'Dropdown') {
    return {
      ...
      isEdited: createIsEdited(propertiesPanel.isSelectEntryEdited, element, property),
      property
    };
  }
  if (type === 'String') {
    ...
    return {
      ...
      isEdited: createIsEdited(propertiesPanel.isTextFieldEntryEdited, element, property),
      property
    };
  }
  if (type === 'Text') {
    ...
    return {
      ...
      isEdited: createIsEdited(propertiesPanel.isTextAreaEntryEdited, element, property),
      property
    };
  }
}
...
function isMessageSupported(element) {
  return ModelUtil.is(element, 'bpmn:ReceiveTask') || ModelUtil.isAny(element, ['bpmn:StartEvent', 'bpmn:EndEvent', 'bpmn:IntermediateThrowEvent', 'bpmn:BoundaryEvent', 'bpmn:IntermediateCatchEvent']) && !!getMessageEventDefinition(element);
}
...
function MessageRef(props) {
  ...
  const bpmnFactory = bpmnJsPropertiesPanel.useService('bpmnFactory');
  const modeling = bpmnJsPropertiesPanel.useService('modeling');
  const translate = bpmnJsPropertiesPanel.useService('translate');
  const messageEventDefinition = getMessageEventDefinition(element);
  const getValue = () => {
...
class ChangeElementTemplateHandler {
  ...
  _updateCamundaFieldProperties(element, oldTemplate, newTemplate, businessObject) {
    ...
    const propertyName = ModelUtil.isAny(businessObject, ['camunda:ExecutionListener', 'camunda:TaskListener']) ? 'fields' : 'values';
...
function LocalVariableAssignment(props) {
  ...
  const bpmnFactory = bpmnJsPropertiesPanel.useService('bpmnFactory'),
    modeling = bpmnJsPropertiesPanel.useService('modeling'),
    translate = bpmnJsPropertiesPanel.useService('translate');
  const getValue = () => {
...
var propertiesProviderModule = {
  __depends__: [translateModule, bpmnJsPropertiesPanel.CamundaPlatformPropertiesProviderModule],
  __init__: ['elementTemplatesPropertiesProvider'],
...
function validate ({
  ...
  function check(node, reporter) {
    if (!ModelUtil.is(node, 'bpmn:FlowElement')) {
      return;
    }
...
function compatibility ({
  ...
  function check(node, reporter) {
    if (ModelUtil.is(node, 'bpmn:Definitions')) {
...
DescriptorBuilder.prototype.build = function () {
  return minDash.pick(this, ['ns', 'name', 'allTypes', 'allTypesByName', 'properties', 'propertiesByName', 'bodyProperty', 'idProperty']);
};
...
Reader.prototype.fromXML = function (xml, options, done) {
  ...
  var model = this.model,
    lax = this.lax;
  var context = new Context(minDash.assign({}, options, {
...
function filterAttributes(props) {
  return minDash.filter(props, function (p) {
    return p.isAttr;
  });
}
...
BpmnModdle.prototype.fromXML = function (xmlStr, typeName, options) {
  ...
  var reader = new Reader(minDash.assign({
    model: this,
    lax: true
  }, options));
...
exports.CloudElementTemplatesCoreModule = coreModule$1;
exports.CloudElementTemplatesLinterPlugin = ElementTemplateLinterPlugin;
exports.CloudElementTemplatesPropertiesProviderModule = index$1;
exports.CloudElementTemplatesValidator = Validator;
exports.ElementTemplatesCoreModule = coreModule;
